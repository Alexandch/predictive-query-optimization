...
`0.8.6` адаптивно оценивает время выполнения, проверяет индексные и структурные
...
- автоматический прогрев и пять read-only замеров при каждом обычном анализе;
- адаптивная оценка времени: XGBoost плюс профиль конкретной БД и точного SQL;
...
Обычный анализ автоматически выполняет один прогрев и пять read-only запусков.
Карточка показывает адаптивную оценку ML + профиль БД, медиану и диапазон этих
замеров; для точного SQL первый устойчивый замер применяется сразу. Это не
обещание точного времени будущего запуска: кэш и нагрузка меняются. «Глубокий
анализ» отдельно измеряет текущий и пробный планы в одной среде. Для решения о
применении оптимизации приоритет имеет эта фактическая проверка.
...
```text
SQL -> read-only прогрев + 5 замеров -> профиль БД -> адаптивная оценка
 |              |
 +-> EXPLAIN + каталоги -> XGBoost + DQN/правила -> рекомендация
                                                   |
                         глубокая проверка -> применить/откатить
```
...
Локальная точечная калибровка выполняется автоматически при каждом нажатии
«Анализировать и измерить»: один прогревочный запуск и пять измерений, после чего
сохраняются медиана и диапазон. Кнопка предварительной пакетной калибровки нужна
только для заполнения общего профиля незнакомой БД. Готовый набор из 28 запросов
находится в `examples/manual-check-queries.sql`.
