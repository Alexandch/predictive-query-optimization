...
уникальных SQL. Текущий артефакт модели получает `R² = 0,9701` на parameter-holdout
и stress-R² новых структур `0,7761`. Простое повторение покрытых параметров почти
исчерпало пользу.
