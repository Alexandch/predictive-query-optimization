...
|---|---:|---:|
| DQN `generic-v2` | 39,29% | 0,1064 |
| случайный выбор | 21,43% | 0,2557 |
| всегда `NOOP` | 14,29% | 0,3023 |

XGBoost сохранил высокий R² на полностью новой предметной области. DQN заметно
превзошёл обе простые стратегии, однако низкая точность `NOOP` показывает, что
агенту требуется больше разнообразных отрицательных примеров. Эти результаты
являются исходной точкой; дообучать модели на данных из данного контроля нельзя.
