-- Vendored from Pagila v3.1.0, commit fef9675714cfba1756df4719b5e36075a7ddf90e.
-- MIT license: database/vendor/pagila/LICENSE.txt
...
-- Pagila originally lives in public, so its function bodies use unqualified
-- relation names. Keep them bound to the isolated project schema.
DO $pagila_search_path$
DECLARE
...
BEGIN
    FOR function_signature IN
        ...
        WHERE n.nspname = 'pagila'
          AND p.prokind = 'f'
    LOOP
        EXECUTE format(
            'ALTER FUNCTION %s SET search_path TO pagila, pg_temp, pg_catalog',
            function_signature
        );
    END LOOP;
END
