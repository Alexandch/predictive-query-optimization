"""Collect development training data from the Pagila domain."""
...
from .dqn_features import GENERIC_V3_ACTION_ENCODING
from .pagila_queries import PagilaQueryGenerator


def main() -> int:
    parser = argparse.ArgumentParser(description="Collect Pagila development data")
    commands = parser.add_subparsers(dest="command", required=True)
    xgb = commands.add_parser("collect-xgb")
    xgb.add_argument("count", type=int)
    xgb.add_argument("output", type=Path)
    xgb.add_argument("--seed", type=int, default=18101)
    ...
    dqn.add_argument("--seed", type=int, default=18101)
    dqn.add_argument("--actions-per-query", type=int, default=3)
    dqn.add_argument("--all-actions", action="store_true")
    dqn.add_argument("--repetitions", type=int, default=2)
    dqn.add_argument("--resume", action="store_true")
    normalize = commands.add_parser("normalize-dqn")
    ...
    if args.command == "normalize-dqn":
        ...
        print(f"Normalized {count} Pagila DQN experiences into {args.output}")
        return 0
    generator = PagilaQueryGenerator(seed=args.seed)
    ...
    if args.command == "collect-xgb":
        ...
        print(f"Collected {len(results)} Pagila XGBoost measurements")
        return 0
    ...
    print(f"Collected {count} Pagila DQN experiences")
    return 0
